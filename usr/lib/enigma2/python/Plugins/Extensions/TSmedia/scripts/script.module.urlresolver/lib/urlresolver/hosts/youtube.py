...
from iTools import printE,printD
import codecs
import json
import re
...
def get_youtube_video_id(value):
    ...
    if query.hostname in ('www.youtube.com', 'youtube.com'):
        ...
        if query.path[:3] == '/v/':
            return query.path.split('/')[2]
    return value
#print 'sys.argv[2]xxx',sys.argv[2]
#video_url= getvideo(sys.argv[2])
#import webbrowser
...
class YouTubeVideoUrl():
	...
	def _download_webpage(self, url, fatal=True):
		...
		try:
			#if ssl.sslContext:
				#urlh = urlopen(url, context=sslsslContext)
			#else:
			urlh = urlopen(url)
		except URLError, e:
			if fatal:
	...
	def extract(self, video_id,quality):
		...
		if 'conn' in video_info and video_info['conn'][0][:4] == 'rtmp':
			...
			if PRIORITY_VIDEO_FORMAT[0] != 38:#config.TSmedia.youtube_vidqual.value:
				createPriorityFormats()
