...
def createPriorityFormats():
	...
	for itag in ['17', '5', '18', '35', '22', '37', '38']:
		...
		if itag == config.TSmedia.youtube_vidqual.value:
			break
...
class YouTubeVideoUrl():
	...
	def _extract_signature_function(self, player_url):###mf
		id_m = re.match(
	...
	def _parse_sig_js(self, jscode):###mf
	...
	def extract(self, video_id):
		...
		if 'conn' in video_info and video_info['conn'][0][:4] == 'rtmp':
			...
			if PRIORITY_VIDEO_FORMAT[0] != config.TSmedia.youtube_vidqual.value:
				createPriorityFormats()
